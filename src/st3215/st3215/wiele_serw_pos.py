...
def main():
    ...
    SCS_MAXIMUM_POSITION_VALUE  = 4500
    ...
    for i in range(3):
        ...
        packetHandler.write1ByteTxRx(ID_C, SMS_STS_MODE, 0)
...
############################################################################################
    ...
    while 1:
        ...
        for i in range(3):
            ...
            # Wysyłanie komendy ruchu do serwa
            scs_comm_result, scs_error = packetHandler.WritePosEx(servo_id, scs_goal_position[index], SCS_MOVING_SPEED, SCS_MOVING_ACC)
            ...
            while 1:
                ...
                if scs_comm_result == COMM_SUCCESS:
                    # Wyświetla pozycję (żebyś widział, czy serwo nr 3 w ogóle zlicza pozycję, czy tylko kręci)
                    print("[ID:%03d] Goal:%d PresPos:%d PresSpd:%d" % (servo_id, scs_goal_position[index], scs_present_position, scs_present_speed))
                ...
                if is_stopped:
                    print(f"Serwo ID: {servo_id} osiągnęło cel.")
                    break 
